...
class TwitterAPICaller: BDBOAuth1SessionManager {    
    static let client = TwitterAPICaller(baseURL: URL(string: "https://api.twitter.com"), consumerKey: "yWqPGmUBxuZP90T8axUiAaAaO", consumerSecret: "55ZHMUgUqfvvVOyNL0LAuv3fnKaj4SIVH38OtiEfoiOS5WKUNx")
    var loginSuccess: (() -> ())?
...
//        print(parameters)
    ...
    func likeTweet(tweetId:Int, success: @escaping () -> (), failure: @escaping (Error) -> ()) {
        ...
        TwitterAPICaller.client?.post(URL, parameters: ["id": tweetId], progress: nil, success: { (URLSessionDataTask, response: Any?) in success()
    ...
    func postTweet(tweetString: String, success: @escaping () -> (), failure: @escaping (Error) -> ()){
        ...
        //get post URL to allow tweet posts
        
        let URL = "https://api.twitter.com/1.1/statuses/update.json"
        
        TwitterAPICaller.client?.post(URL, parameters: ["status": tweetString], progress: nil, success: { (URLSessionDataTask, response: Any?) in success()
